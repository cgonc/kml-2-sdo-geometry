...
public class OraReader {
	...
	public OraReader(GeometryFactory gf) {
		if(gf == null) throw new NullPointerException("Geometry Factory may not be Null");
	...
	private Polygon readPolygon(OraGeom oraGeom, int elemIndex) {
		...
		Polygon poly = geometryFactory.createPolygon(exteriorRing, GeometryFactory.toLinearRingArray(holeRings));
		return poly;
	}
	...
	private LinearRing readLinearRing(OraGeom oraGeom, int elemIndex) {
		...
		if(interpretation == OraGeom.INTERP.POLYGON){
			...
			ring = geometryFactory.createLinearRing(new Coordinate[]{min, new Coordinate(max.x, min.y), max, new Coordinate(min.x, max.y), min});
		}
		return ring;
	}
	...
	private LineString readLine(OraGeom oraGeom, int elemIndex) {
		...
		LineString line = geometryFactory.createLineString(extractCoords(oraGeom, elemIndex));
		return line;
	}
	...
	private static void checkOrdinates(int[] elemInfo, int elemIndex, int startOffset, int ordLen, String geomType) {
		if(startOffset > ordLen) throw new IllegalArgumentException(
				"STARTING_OFFSET " + startOffset + " inconsistent with ORDINATES length " + ordLen + " (Element " + elemIndex + " in SDO_ELEM_INFO " + OraGeom.toStringElemInfo(elemInfo) + ")");
	}
