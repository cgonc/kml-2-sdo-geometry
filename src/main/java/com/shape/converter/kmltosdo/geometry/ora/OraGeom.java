...
class OraGeom {
	...
	public boolean isEqual(OraGeom og) {
		...
		if(!isEqual(point, og.point)) return false;
		// assume is defined by elemInfo and ordinates
		if(!isEqual(elemInfo, og.elemInfo)) return false;
		if(!isEqual(ordinates, og.ordinates)) return false;
		return true;
	}
	...
	private Object toStringPointType() {
		...
		return "SDO_POINT_TYPE(" + number(point[0]) + "," + number(point[1]) + "," + number(point[2]) + ")";
	}
	...
	public int ordinateLen() {
		if(ordinates != null) return ordinates.length;
		return 0;
	}
