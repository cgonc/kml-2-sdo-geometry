package com.shape.converter.kmltosdo.geometry.ora;
...
public class OraWriter {
	...
	public void setDimension(int outputDimension) {
		if(outputDimension < 2) throw new IllegalArgumentException("Output dimension must be >= 2");
		this.outputDimension = outputDimension;
	}
	...
	public STRUCT write(Geometry geom, OracleConnection connection) throws SQLException {
		...
		if(geom == null || geom.isEmpty() || geom.getCoordinate() == null) return createEmptySDOGeometry(connection);
		...
		if(oraGeom.point == null){
			SDO_ELEM_INFO = OraUtil.toARRAY(oraGeom.elemInfo, OraGeom.TYPE_ELEM_INFO_ARRAY, connection);
			SDO_ORDINATES = OraUtil.toARRAY(oraGeom.ordinates, OraGeom.TYPE_ORDINATE_ARRAY, connection);
	...
	private int writeElement(Geometry geom, int dim, int offset, List elemTriplets, List ordGeoms) {
		...
		throw new IllegalArgumentException(
				"Cannot encode JTS " + geom.getGeometryType() + " as SDO_ELEM_INFO " + "(Limited to Point, Line, Polygon, GeometryCollection, MultiPoint," + " MultiLineString and MultiPolygon)");
	}
	...
	private boolean isEncodeAsPointType(Geometry geom) {
		...
		if(geom instanceof Point && (lrsDim(geom) == 0) && outputDimension <= 3) return true;
	...
	private int dimension(Geometry geom) {
		if(outputDimension != OraGeom.NULL_DIMENSION) return outputDimension;

		//TODO: check dimension of a geometry CoordinateSequence to determine dimension
		int d = Double.isNaN(geom.getCoordinate()
								 .getZ()) ? 2 : 3;
		return d;
	}
